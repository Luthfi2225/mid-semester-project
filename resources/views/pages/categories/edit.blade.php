    ...
    <div class="border border-gray-400 dark:border-black rounded-xl">
        <div class="bg-white dark:bg-[#191a1b] rounded-xl">
            <div class="p-6 border-b border-gray-400 dark:border-black">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui informasi kategori yang sudah ada.</p>
            </div>
            ...
            <form action="{{ route('categories.store') }}" method="POST" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    @if ($isParent)
                        <select disabled class="disabled:cursor-not-allowed w-full pl-4 py-2 font-bold bg-gray-300 dark:bg-[#343638] border border-gray-400 dark:border-black text-gray-400 rounded-lg appearance-none bg-no-repeat bg-right pr-10 outline-none transition duration-400"
                    ...
                    @else
                        <select name="parent_id" class="w-full pl-4 py-2 border border-gray-400 dark:border-black focus:border-blue-500 hover:bg-gray-400 dark:hover:bg-gray-600 focus:bg-white dark:focus:bg-[#191a1b] rounded-lg outline-none appearance-none bg-no-repeat bg-right pr-10 transition duration-400"
                                                style="background-image: url('data:image/svg+xml;charset=utf-8,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 fill=%27none%27 viewBox=%270 0 20 20%27%3E%3Cpath stroke=%27%236b7280%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27 stroke-width=%271.5%27 d=%27m6 8 4 4 4-4%27/%3E%3C/svg%3E'); background-size: 1.5em 1.5em; background-position: right 0.20rem center;">
                            @foreach($parent_categories as $cat)
                                <option value="{{ $cat->id }}" {{ $categories->parent_id == $cat->id ? 'selected' : ''}} class="bg-gray-200 dark:bg-[#35373a]">
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <button type="reset" class="px-6 py-2 border border-gray-400 dark:border-black rounded-lg text-gray-600 dark:text-white hover:bg-gray-400 dark:hover:bg-gray-600 transition duration-400">
                        Kosongkan Form
                    </button>
